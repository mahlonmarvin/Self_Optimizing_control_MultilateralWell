...
po2bmC1 = []; po3bmC1 = []; po4bmC1 = []; pw2bmC1 = []; Yw1 = []; pw3bmC1 = []; pw4bmC1 = []
uw1 = []; NPV1bm=0; NPVbmC1=[]; l=1;
ubm1C1=[];
...
while t < T,
   ...
   NPV1bm=NPV1bm+dT*(-(WatIn1)*wi-(WatPr2+WatPr3+WatPr4)*wp+(OilPr2+OilPr3+OilPr4)*op)/(1+b)^t;
...
figure
subplot(2,3,1),
   plot(1:n,3600*24*po2bmC1(:,1),1:n,3600*24*po2ocC1(:,1),'g', 1:n, 3600*24*po2socC1(:,1),'r' )
...
subplot(2,3,3),
   ...
   title('Oil production in well P1');
subplot(2,3,4),
   plot(1:n,3600*24*pw2bmC1(:,1),1:n,3600*24*pw2ocC1(:,1),'g', 1:n,3600*24*pw2socC1(:,1),'r')
...
% total production
Tvec = (dT : dT : T)';
oilT1 = cumtrapz ([0; Tvec], [zeros([1, 1]); po3bmC1]);
oilT2 = cumtrapz ([0; Tvec], [zeros([1, 1]); po4bmC1]);
wat1 = cumtrapz ([0; Tvec], [zeros([1, 1]); pw3bmC1]);
wat2 = cumtrapz ([0; Tvec], [zeros([1, 1]); pw4bmC1]);
%%
